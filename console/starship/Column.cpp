...
bool Column::checkCollision(Vector3 ship) const
{
...
std::vector<Column> Column::generateRandomColumns(size_t n, Vector3 map_size, bool horizontal)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (horizontal && i % 20 == 0) {
            ...
            columns.emplace_back(polePosition, poleSize, color);
            polePosition.x *= -1.0F;
            columns.emplace_back(polePosition, poleSize, color);
        } else {
            Vector3 position {
